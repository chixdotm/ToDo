package ToDo;

public class Settings {

    public static final String USAGE = "Parancssori Todo applikáció\n" +
            "=============================\n" +
            "\n" +
            "Parancssori argumentumok:\n" +
            "    -u-  Kiírja USAGE-t\n" +
            "    -n   New StickyNote\n" +
            "    -t   Create StickyNote\n" +
            "    -w   Write StickyNote\n" +
            "    -L   List StickyNote\n" +
            "    -s   List StickyNotes\n" +
            "    -l-  KiListázza a feladatokat\n" +
            "    -a-  Új feladatot ad hozzá\n" +
            "    -r   Eltávolít egy feladatot\n" +
            "    -c   Teljesít egy feladatot\n" +
            "    -e-  Exit";

    public static final String USAGE_OPERATION = "-u";
    public static final String NEW_STICKY_OPERATION = "-n";
    public static final String CREATE_STICKY_OPERATION = "-t";
    public static final String WRITE_OPERATION = "-w";
    public static final String LIST_STICKY_OPERATION = "-L";
    public static final String LIST_OPERATION = "-l";
    public static final String ADD_OPERATION = "-a";
    public static final String EXIT_OPERATION = "-e";

    public static final String STICKY_PATH = "Sticky/";


}
